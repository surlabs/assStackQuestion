...
class assStackQuestionTestGUI
{
	...
	public function fillTestCaseHeader($test)
	{
		...
		if ($this->mode)
		{
			...
			if (isset($unit_test_results["test_passed"]))
			{
				...
				return $this->getPlugin()->txt("ut_testcase_name") . " class.assStackQuestionTestGUI.php" . $test->getTestCase() . " " . '<img src="' . $icon . '" /> ';

			} else
			{
				return $this->getPlugin()->txt("ut_testcase_name") . " class.assStackQuestionTestGUI.php" . $test->getTestCase();
			}
		} else
		{
			return $this->getPlugin()->txt("ut_testcase_name") . " class.assStackQuestionTestGUI.php" . $test->getTestCase();
	...
	public function editTestcaseForm($testcase_name, $question_inputs, $question_prts)
	{
		...
		$form_container->setHeading($this->getPlugin()->txt("ut_testcase_name") . " class.assStackQuestionTestGUI.php" . $testcase_name);
		...
		{
			$expected_score = new ilTextInputGUI($this->getPlugin()->txt('ut_expected_score_for') . ' class.assStackQuestionTestGUI.php' . $prt_name, 'score_' . $prt_name);
		...
		foreach ($question_prts as $prt_name => $q_prt)
		{
			$expected_penalty = new ilTextInputGUI($this->getPlugin()->txt('ut_expected_penalty_for') . ' class.assStackQuestionTestGUI.php' . $prt_name, 'penalty_' . $prt_name);
			foreach ($unit_test->getTestExpected() as $prt => $expected)
			{
				if ($prt_name == $expected->getTestPRTName())
				{
		...
		foreach ($question_prts as $prt_name => $q_prt)
		{
			$expected_answernote = new ilTextInputGUI($this->getPlugin()->txt('ut_expected_answernote_for') . ' class.assStackQuestionTestGUI.php' . $prt_name, 'answernote_' . $prt_name);
			foreach ($unit_test->getTestExpected() as $prt => $expected)
			{
				if ($prt_name == $expected->getTestPRTName())
				{
