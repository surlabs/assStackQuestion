...
class assStackQuestionMoodleImport
{
    ...
    public function importQuestions($raw_data)
    {
        ...
        //Check if is PHP 7.2 in case it is, data need some extra formatting
        if (assStackQuestionUtils::_isPhP72()) {
            $raw_data = $this->php72Format($raw_data['question']);
        }
            ...
            if (is_array($question_data['prt'])) {
                foreach ($question_data['prt'] as $prt_raw) {
                    ...
                    if (isset($prt_raw['feedbackvariables'][0]["_content"])) {
                        $prt_data['feedbackvariables'] = $prt_raw['feedbackvariables'][0]["_content"];
                    } elseif (isset($prt_raw['feedbackvariables'][0]["text"][0]["_content"])) {
                        $prt_data['feedbackvariables'] = $prt_raw['feedbackvariables'][0]["text"][0]["_content"];
                    } elseif (isset($prt_raw['feedbackvariables'][0]["text"])) {
                        $prt_data['feedbackvariables'] = $prt_raw['feedbackvariables'][0]["text"];
                    }
                    ...
                    if (is_array($prt_raw['node'])) {
                        foreach ($prt_raw['node'] as $node_raw) {
                            ...
                            } elseif (isset($node_raw['truefeedback'][0]["text"])) {
                                $node_data['truefeedback'][0]["text"] = $node_raw['truefeedback'][0]["text"];
                            } else {
                            ...
                            } elseif (isset($node_raw['falsefeedback'][0]["text"])) {
                                $node_data['falsefeedback'][0]["text"] = $node_raw['falsefeedback'][0]["text"];
                            } else {
