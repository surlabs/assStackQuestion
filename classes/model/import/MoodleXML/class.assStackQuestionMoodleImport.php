...
class assStackQuestionMoodleImport
{
	...
	private function createStandardQuestion($data)
	{
		...
		$this->getQuestion()->setQuestion(ilUtil::secureString($questiontext, true, $this->getRTETags()));
	...
	private function getOptionsFromXML($data)
	{
		...
		$specificfeedback = $this->replaceMediaObjectReferences($data['specificfeedback'][0]['text'], $mapping);
	...
	private function getPRTNodesFromXML($data, $prt_name)
	{
		...
		foreach ($data as $prt_node) {
			...
			$truefeedback = $this->replaceMediaObjectReferences($prt_node['truefeedback'][0]['text'], $mapping);
	...
	private function getExtraInfoFromXML($data)
	{
		...
		$how_to_solve = $this->replaceMediaObjectReferences($data['generalfeedback'][0]['text'], $mapping);
