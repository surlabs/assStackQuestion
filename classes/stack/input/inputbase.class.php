...
abstract class stack_input
{
	 ...
	 * @var array
	 */
	protected static $allparameternames = array('mustVerify', 'showValidation', 'boxWidth', 'boxHeight', 'strictSyntax', 'insertStars', 'syntaxHint', 'syntaxAttribute', 'forbidWords', 'allowWords', 'forbidFloats', 'lowestTerms', 'sameType');
	...
	public function set_parameter($parameter, $value)
	{
		...
		{
			throw new stack_exception('stack_input: setting parameter ' . $parameter . ' which does not exist for inputs of type ' . get_class($this));
		}
	 ...
	 *    fim #14: make this method public in order to be called by ilias factory
	 */
	...
	public function validate_parameter($parameter, $value)
	{
		if (!$this->is_parameter_used($parameter))
		{
			throw new stack_exception('stack_input: trying to validate parameter ' . $parameter . ' which does not exist for inputs of type ' . get_class($this));
		}
	...
	public function validate_student_response($response, $options, $teacheranswer, $forbiddenkeys, $ajaxinput = false)
	{
		...
		if ('checktype' == $validationmethod || 'units' == $validationmethod || 'unitsnegpow' == $validationmethod)
		{
			...
			list($tvalid, $terrors, $tmodifiedcontents, $tcaslines) = $this->validate_contents($tcontents, $forbiddenkeys, $localoptions);
		} else
		{
			$tcaslines = array();
		}
		...
		foreach ($caslines as $index => $cs)
		{
			...
			{
				$cs->set_cas_validation_casstring($this->name . $index, $this->get_parameter('forbidFloats', false), $this->get_parameter('lowestTerms', false), $ta, $ivalidationmethod, $this->get_parameter('allowWords', ''));
				$sessionvars[] = $cs;
			}
		}
		...
		$answer->set_cas_validation_casstring($this->name, $this->get_parameter('forbidFloats', false), $this->get_parameter('lowestTerms', false), $teacheranswer, $validationmethod, $this->get_parameter('allowWords', ''));
	...
	protected function validate_contents($contents, $forbiddenkeys, $localoptions)
	{
		...
		foreach ($contents as $index => $val)
		{
			...
			if (2 == $this->get_parameter('insertStars', 0) || 5 == $this->get_parameter('insertStars', 0))
			{
				$val = stack_utils::make_single_char_vars($val, $localoptions, $this->get_parameter('strictSyntax', true), $this->get_parameter('insertStars', 0), $this->get_parameter('allowWords', ''));
			}
