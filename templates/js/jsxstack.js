stack_jxg = new function () {
    return {
        find_input_id: function (divid, name) {
            var tmp = document.getElementById(divid);

            //find ancestor "ilc_question_Standard"
            while (tmp && !(tmp.classList.contains("ilc_question_Standard"))) {
                tmp = tmp.parentElement;
            }

            if (!tmp) {
                alert("ancestor in class ilc_question_Standard not found");
                return null;
            }
